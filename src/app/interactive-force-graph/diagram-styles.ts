...
const nodeStateSearch: NodeStateProperties = {
  ...
  shapeSuperformula: new Superformula()
    .type(() => 'gear')
    .size((d) => 2 * utils.getHighlightedRadius(d)),
};
...
export class DiagramStyles {
  ...
  public applyNodeSearch(nodeEles: d3.Selection<SVGGElement, Node, d3.BaseType, any>) {
    ...
    nodeShape
      ...
      .attr('d', (d: unknown) =>
        bigSuperdupaPath
          .size((bd) => 5 * utils.getHighlightedRadius(bd))
          .getPath(d as Node)
      )
    ...
    nodeText
      ...
      .style('opacity', 1)
      .attr('transform', (d: unknown) => utils.nodeTextShiftRight(d as Node, nodeStateSearch.nodeTextShiftMultiplier));
  }
  ...
  public applyNodeHighlight(nodeEles: d3.Selection<SVGGElement, Node, d3.BaseType, any>) {
    ...
    nodeText
      ...
      .duration(durationLong)
      .style('opacity', 1)
      .attr('transform', (d: unknown) => utils.nodeTextShiftRight(d as Node, nodeStateHighlight.nodeTextShiftMultiplier));
  }
  ...
  public applyNodeHighlightedNeighbour(nodeEles: d3.Selection<SVGGElement, Node, d3.BaseType, any>) {
    ...
    nodeShape
      ...
      .attr('stroke', () => constants.everythingElseColor)
      .attr('fill', () => constants.everythingElseColor)
      .style('stroke-opacity', 1)
      .style('fill-opacity', 1);
  ...
  public applyLinkGradientDefault(linkGradientEles: d3.Selection<SVGLinearGradientElement, Link, d3.BaseType, any>) {
    ...
    stopTwo
      .attr('offset', '100%')
      .attr('stop-color', constants.linkGradientColorEnd);
  }
