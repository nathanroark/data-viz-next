...
import type { Link, Node, Hull } from "./data-interfaces";
...
export function load(
  ...
  // Load the data
  const dataUrl = "./data/my_services_graph.json"; //process.env.REACT_APP_DATA_SERVICES_URL!;
  ...
  d3.json(dataUrl).then((uResponse: unknown) => {
    ...
    if (initialHighlightedNodeNames.length > 0) {
      for (const highlightedNodeName of initialHighlightedNodeNames) {
        ...
        const nodeIdx = utils.findIndex(nodes, (hn: Node) =>
          utils.strEquals(utils.getNodeNameAndGroup(hn), highlightedNodeName),
        );
        if (nodeIdx > -1) {
          // highlight it
...
export function searchForNodes(searchText: string) {
  ...
  showOnlyHighlighted(
    diagramStyles.showOnlyHighlighted && highlightedNodes.length > 0,
  );
}
...
let linkGradients: d3.Selection<
...
function prepare() {
  diagramWidth =
    Math.floor(
      Number(
        window
          .getComputedStyle(document.getElementById("diagram")!)
          .width!.replace("px", ""),
      ),
    ) - 10;
  diagramHeight =
      ...
      d3
        ...
        .distance((slDatum: d3.SimulationLinkDatum<{}>) => {
          ...
          if (utils.isNodeNotString(n1) && utils.isNodeNotString(n2)) {
            const combinedRadiuses: number =
              utils.getRadius(n1) + utils.getRadius(n2);
...
function updateSimulation() {
  ...
  simulation.nodes(nodes).on("tick", () => {
    ...
    for (const d of nodes) {
      if (d.group) {
        const biggestNode =
          biggestNodePerGroup[utils.strToLowerOrEmpty(d.group)];
        if (biggestNode !== d) {
          ...
          if (l !== r) {
            ...
            const alphaMultiplier =
              -0.9 * Math.pow(alpha - 1, 2) - 0.9 * (alpha - 1) + 0.7;
            const l1 = l - r;
            const t = l1 / l;
...
function dragged(
  evt: any,
  d: Node,
  nodeEles: d3.Selection<SVGGElement, Node, d3.BaseType, any>,
) {
  if (hasForceSimulation) {
...
function onNodeClick(evt: any, d: Node) {
  ...
  const highlightedNodeIdx = utils.findIndex(highlightedNodes, (hn: Node) =>
    utils.strEquals(
      utils.getNodeNameAndGroup(hn),
      utils.getNodeNameAndGroup(d),
    ),
  );
...
function ungroupNodes(d: Node) {
  ...
  for (const k1 of d.nodes!) {
    k1.x =
      d.x! +
      (1 + Math.random()) *
      (utils.getRadius(d) * 1.3 + utils.getRadius(k1)) *
      (Math.random() < 0.5 ? -1 : 1);
    k1.y =
      d.y! +
      (1 + Math.random()) *
    ...
    for (const lLink of links) {
      if (
        ...
        (utils.strEquals(lLink.targetChild, childMNode.name) ||
          (utils.isNodeNotString(lLink.targetChild) &&
            utils.strEquals(lLink.targetChild.name, childMNode.name)))
      ) {
      ...
      if (
        ...
        (utils.strEquals(lLink.sourceChild, childMNode.name) ||
          (utils.isNodeNotString(lLink.sourceChild) &&
            utils.strEquals(lLink.sourceChild.name, childMNode.name)))
      ) {
...
function regroupNodes(d: Node) {
  ...
  for (let k = 0; k < nodes.length; ++k) {
    ...
    if (utils.doGroupsMatch(kNode, d)) {
      ...
      for (const mLink of links) {
        if (
          (utils.strEquals(mLink.target, kNode.name) ||
            (utils.isNodeNotString(mLink.target) &&
              utils.strEquals(mLink.target.name, kNode.name))) &&
          (!mLink.targetChild ||
            (utils.isNodeNotString(mLink.targetChild)
              ? !utils.strEquals(mLink.targetChild.group, kNode.name)
              : true))
        ) {
        ...
        if (
          (utils.strEquals(mLink.source, kNode.name) ||
            (utils.isNodeNotString(mLink.source) &&
              utils.strEquals(mLink.source.name, kNode.name))) &&
          (!mLink.sourceChild ||
            (utils.isNodeNotString(mLink.sourceChild)
              ? !utils.strEquals(mLink.sourceChild.group, kNode.name)
              : true))
        ) {
      ...
      const highlightedNodeIdx = utils.findIndex(highlightedNodes, (hn: Node) =>
        utils.strEquals(
          utils.getNodeNameAndGroup(hn),
          utils.getNodeNameAndGroup(kNode),
        ),
      );
      if (highlightedNodeIdx > -1) {
        // then remove node
  ...
  for (let m1 = 0; m1 < links.length; m1++) {
    ...
    if (
      utils.strEquals(m1Link.target, d.group) &&
...
function highlightNodes() {
  highlightedNodesChangedCallback(
    highlightedNodes.map((n) => utils.getNodeNameAndGroup(n)),
  );
  ...
  const matchedNodes = searchUtils.GetMatchedNodes(
    matchedNodesData,
    nodeElements,
  );
  const matchedLinks = searchUtils.GetMatchedLinks(
    matchedNodesData,
    linkElements,
    diagramStyles.showOnlyHighlighted,
  );
  const matchedLinkGradients = searchUtils.GetMatchedLinkGradients(
    matchedNodesData,
    linkGradients,
    diagramStyles.showOnlyHighlighted,
  );
  const unmatchedLinks = searchUtils.GetUnmatchedLinks(
    matchedNodesData,
    linkElements,
    diagramStyles.showOnlyHighlighted,
  );
  const unmatchedLinkGradients = searchUtils.GetUnmatchedLinkGradients(
    matchedNodesData,
    linkGradients,
    diagramStyles.showOnlyHighlighted,
  );
  const neighbourNodes = searchUtils.GetNeighbourNodes(
    matchedNodesData,
    matchedLinks.data(),
    nodeElements,
  );
  const highlightedAndNeighbourNodesData = matchedNodesData.concat(
    neighbourNodes.data(),
  );
  const unmatchedNodes = searchUtils.GetUnmatchedNodes(
    highlightedAndNeighbourNodesData,
    nodeElements,
  );
  const matchedHulls = searchUtils.GetMatchedHulls(
    matchedNodesData,
    hullElements,
  );
  const unmatchedHulls = searchUtils.GetUnmatchedHulls(
    matchedHulls.data(),
    hullElements,
  );

  if (hasSearchedForNodes === true) diagramStyles.applyNodeSearch(matchedNodes);
  else diagramStyles.applyNodeHighlight(matchedNodes);
  ...
  if (nodeOrLink.details) {
    ...
    if (tableData.length > 0) {
      tableElement
        ...
        .enter()
        .append("td")
        .text(((d: string) => d) as unknown as string);
    }
  } else if (
    !utils.isLinkNotNode(nodeOrLink) &&
    nodeOrLink.group &&
    nodeOrLink.nodes
  ) {
    // group
